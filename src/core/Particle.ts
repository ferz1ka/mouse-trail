...
export class Particle {
  ...
  constructor(x: number, y: number, color: string, mouseVx: number = 0, mouseVy: number = 0) {
    this.x = x;
    this.y = y;
    ...
    this.size = config.randomizeSize ? Math.random() * config.size + config.size / 2 : config.size;
